...
- [x] **Goal Understanding**: Interactive Goal Profile interview & goal-weighted evaluation matrices.
- [x] **Hardware Feasibility**: Live throughput probing, compute tiers (`T0`–`T3`), memory profiling.
- [x] **Memory-Optimized Training**: Adaptive layer streaming and memory management for local GPUs.
- [x] **Dataset Intelligence**: Strict Reference Mode, PII & credential redaction, exact/fuzzy deduplication.
- [x] **Tokenizer Engine**: Model-aware streaming tokenization, sequence distributions, context-fit analysis.
...
- [x] **Regression Shipping Gate**: `myai ship` with bundled offline test suites and evidence receipts.
...
- [x] **Export Containment Gate**: Security verification & standalone ZIP package with zero-dependency Luminous Web Chat UI.
...
- [ ] **Compliance Verification**: Automated audit reports for local and air-gapped environments.
